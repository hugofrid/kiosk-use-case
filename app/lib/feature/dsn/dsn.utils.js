...
const getLastBloc = (array: string[]) => {
...
export const DSNFileReader = async (content: string) => {
  ...
  for (let row of rows) {
    if (
      /^S[0-9]{2}\.G[0-9]{2}\.[0-9]{2}\.[0-9]{3},'.*'/.test(row)
...
export const DSNDataReader = async (dataDSN: DataDSN) => {
  // data
  let declaration: Partial<DeclarationBloc> = {}
  let errors: DataDSN["errors"] = []
